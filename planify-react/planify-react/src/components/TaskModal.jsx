...
export function TaskModal({ isOpen, onClose, onSave, day, taskToEdit }) {
  ...
  // Efeito para preencher os dados quando for EDITAR
  useEffect(() => {
    if (isOpen) {
        if (taskToEdit) {
            // Se tem tarefa para editar, preenche os campos
            setTime(taskToEdit.time || '');
            setActivity(taskToEdit.text || '');
        } else {
            // Se é nova tarefa, limpa os campos
            setTime('');
            setActivity('');
        }
    }
    ...
    <div style={styles.overlay}>
      <div style={styles.modal}>
        <div style={styles.header}>
            ...
            <h3>
                {taskToEdit ? 'Editar Tarefa' : `Nova tarefa para ${day ? day.toUpperCase() : '...'}`}
            </h3> 
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
            ...
            <button type="submit" style={styles.saveBtn}>
                {taskToEdit ? 'Atualizar Atividade' : 'Salvar Atividade'}
            </button>
        </form>
      </div>
    </div>
...
const styles = {
  ...
  modal: {
    ...
    padding: '40px',
    borderRadius: '24px',
    width: '90%', maxWidth: '480px', color: 'white'
  },
  ...
  input: {
    padding: '14px', background: '#0f0f0f', border: '1px solid rgba(255, 255, 255, 0.1)',
    borderRadius: '8px', color: 'white', outline: 'none', width: '100%', fontSize: '1rem',
  },
  saveBtn: {
